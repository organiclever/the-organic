...
def _show_summary(argv, events, verbose=False):
    ...
    run_command_info = "[{}] Running: {}".format(time_stamp, highlight(command))
    if not events:
    ...
    if verbose:
        ...
        for event, src, dest in events:
            ...
            lines.append(
                "  {} {}".format(
                    event, highlight(src + (" -> " + dest if dest else ""))
                )
            )
        lines.append("")
        ...
        for event, src, dest in events:
            lines.append(
                "{} detected: {}".format(
                    EVENT_NAMES[event], bright(src + (" -> " + dest if dest else ""))
                )
            )
        lines.append("")
        lines.append(run_command_info)
...
def _split_recursive(directories, ignore):
    ...
    for directory in directories:
        ...
        filtered = [
            ...
            if not any(samepath(os.path.join(directory, d), subdir) for d in ignore)
        ]
