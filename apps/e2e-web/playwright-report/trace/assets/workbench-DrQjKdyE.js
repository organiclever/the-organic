...
class D0 {
  ...
  regexToString(t) {
    ...
    return `re.compile(r"${so(t.source)
      .replace(/\\\//, "/")
      .replace(/"/g, '\\"')}"${n})`;
  }
  toCallWithExact(t, n, r) {
...
const Ka = ({
  ...
  yi = ({
    ...
    const [f, g] = _n(),
      [y] = b.useState(
        hy(
          () => import("./codeMirrorModule-C6p3E9Zg.js"),
          __vite__mapDeps([0, 1]),
          import.meta.url
        ).then((m) => m.default)
      ),
...
function gw(e, t) {
  ...
  for (const r of e)
    switch (r.name) {
      ...
      default:
        throw new Error(
          `Unknown attribute "${r.name}", must be one of ${Op.map(
            (i) => `"${i}"`
          ).join(", ")}.`
        );
    }
...
function Gw(e, t, n) {
  ...
  if (t.nodeName === "IFRAME") {
    for (const o of ["name", "title"])
      t.getAttribute(o) &&
        r.push({
          engine: "css",
          selector: `${Ye(t.nodeName.toLowerCase())}[${o}=${kr(
            t.getAttribute(o)
          )}]`,
          score: $w,
        });
    return (
      t.getAttribute(n.testIdAttributeName) &&
        r.push({
          engine: "css",
          selector: `[${n.testIdAttributeName}=${kr(
            t.getAttribute(n.testIdAttributeName)
          )}]`,
          score: Xc,
        }),
  ...
  if (
    (t.getAttribute(n.testIdAttributeName) &&
      r.push({
        ...
        selector: `[${n.testIdAttributeName}=${Te(
          t.getAttribute(n.testIdAttributeName),
          !0
        )}]`,
        score: Xc,
  ...
  return (
    ...
    t.getAttribute("name") &&
      ...
      r.push({
        engine: "css",
        selector: `${Ye(t.nodeName.toLowerCase())}[name=${kr(
          t.getAttribute("name")
        )}]`,
        score: Ho,
      }),
    ["INPUT", "TEXTAREA"].includes(t.nodeName) &&
      ...
      r.push({
        engine: "css",
        selector: `${Ye(t.nodeName.toLowerCase())}[type=${kr(
          t.getAttribute("type")
        )}]`,
        score: Ho,
      }),
  ...
  strictModeViolationError(t, n) {
    ...
    return (
      r.length < n.length &&
        i.push(`
    ...`),
      this.createStacklessError(`strict mode violation: ${Yt(
        this._sdkLanguage,
        Sn(t)
